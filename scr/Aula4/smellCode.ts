...
if (emprestados >= limite) { console.log("Limite atingido"); return; }
 livro[3] = "EMPRESTADO";
 usuario[3] = String(emprestados + 1);
const hoje = new Date(); const devolucao = new Date(hoje);
 devolucao.setDate(devolucao.getDate() + 14);
this.emprestimos.push([isbn, emailUsuario,
 hoje.toISOString().split('T')[0], devolucao.toISOString().split('T')[0], "ATIVO"]);
this.enviarEmail(emailUsuario, "Empréstimo", "Você emprestou: " + livro[0]);
 }
gerarRelatorio(tipo: string): string {
let sb = "";
if (tipo === "LIVROS") { sb += "=== RELATÓRIO DE LIVROS ===\n";
for (const l of this.livros) { sb += l[0] + " | " + l[1] + " | " + l[3] + "\n"; }
 } else if (tipo === "USUARIOS") { sb += "=== RELATÓRIO DE USUÁRIOS ===\n";
for (const u of this.usuarios) { sb += u[0] + " | " + u[1] + " | Tipo: " + u[2] + "\n"; }
 } else if (tipo === "EMPRESTIMOS") { sb += "=== RELATÓRIO DE EMPRÉSTIMOS ===\n";
for (const e of this.emprestimos) { sb += "ISBN: " + e[0] + " | Usuário: " + e[1] + "\n"; }
 }
return sb;
 }
